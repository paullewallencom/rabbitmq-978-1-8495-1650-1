...
public class BenchConsumer extends ReliableClient {
	...
	int prefetch_count;
	boolean autoAck;
	boolean print_thread_consumer;
	
	private ExecutorService exService;
	
	
	BenchConsumer(int _threadNumber,int _prefetch_count,boolean _autoAck,boolean _print_thread_consumer)  {
		super();
		...
		print_thread_consumer = _print_thread_consumer;
	}
			...
			exService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						internalChannel.basicConsume(Constants.queue, autoAck, new Consumer() {
							@Override
							...
							public void handleDelivery(String consumerTag,Envelope envelope, BasicProperties properties,
							  ...
							  if (print_thread_consumer)
									System.out.println("- thread- number : " +threadnum +" ThreadID: " + Thread.currentThread().getId());
